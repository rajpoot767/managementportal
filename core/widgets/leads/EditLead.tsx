...
const EditLead: React.FC<WidgetProps> = ({ params }) => {
  ...
  const onUpdateClick = useCallback(async (formState: FormState<UpdateCommentRequest>): Promise<ActionResponse<any>> => {
    return new LeadService().updateLeadComment(formState.inputValues);
  }, []);
  ...
  const sections: DataFormSection[] = [
    {
      sectionRows: [
        ...
        {
          grow: true,
          elements: [{ name: "comment", controlType: InputControlType.lineTextInput, attributes: { label: "Comment" } }],
        },
      ],
    },
  ];
    ...
    <div className="h-full container p-4 bg-white max-h-full overflow-y-scroll">
      ...
      <div className="h-full rounded-md mb-3 bg-white">
        <DataForm2
          ...
          onDeleteClassname="flex items-center justify-center px-6 py-3 text-white bg-alternate-700 hover:bg-alternate-600 rounded-md shadow-md hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-success-400 focus:ring-offset-2 transition-transform duration-300"
          deleteButtonTitle="Close Lead"
          onDeleteConfirmationMessage="Are you sure to close this lead?"
        />
      </div>
    </div>
  );
