...
class ServiceClient {
  ...
  private handleFetchError(error: any): {
    ...
    if (serverApiError) {
      console.log(serverApiError.data);
  ...
  async post(path: string, data: any): Promise<ActionResponse<any>> {
    const request = async () => {
  ...
  async getSingle<T>(
    ...
    const request = async () => {
      ...
      return await this.fetchJsonWithCache<SingleResponse<T>>(fullPath, config);
    };
    try {
      return await this.handleRequest(request);
    } catch (error: any) {
  ...
  async get<T>(
    ...
    const request = async () => {
      const fullPath = this.buildFullPath(path, params);
      const config = this.getConfig();
      return await this.fetchJsonWithCache<QueryResponse<T>>(fullPath, config);
    };
    try {
      return await this.handleRequest(request);
    } catch (error: any) {
